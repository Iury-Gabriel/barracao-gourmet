...
import { ArrowLeft, Clock, MapPin, Phone, User, CreditCard, Ticket, Truck, Barcode, Printer } from "lucide-react";
...
type PedidoDetailDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  pedidoId: string | null;
};

export function PedidoDetailDialog({ open, onOpenChange, pedidoId }: PedidoDetailDialogProps) {
  const queryClient = useQueryClient();
  const [novoStatus, setNovoStatus] = useState("");

  const { data: pedido, isLoading } = useQuery({
    queryKey: ["pedido", pedidoId],
    queryFn: () => api.get<any>(`/api/pedidos/${pedidoId}`),
    enabled: open && !!pedidoId,
    refetchInterval: 10000,
  });

  const atualizarStatus = useMutation({
    mutationFn: (status: string) => api.patch(`/api/pedidos/${pedidoId}/status`, { status }),
    onSuccess: () => {
  ...
  const cancelar = useMutation({
    mutationFn: () => api.delete(`/api/pedidos/${pedidoId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pedidos"] });
  ...
  const cfg = pedido ? (STATUS_CONFIG[pedido.status] ?? { label: pedido.status, color: "bg-gray-100 text-gray-600" }) : null;
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => imprimirPedido(pedido)} title="Imprimir cupom">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Card>
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  {pedido.observacoes && (
                    <div className="bg-muted rounded-lg p-3 text-muted-foreground italic mt-2">
                      {pedido.observacoes}
                    </div>
                  )}
                </CardContent>
